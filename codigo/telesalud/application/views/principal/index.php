					...
					<?php if (count($validarDoctor) == 0) { ?>
						<div class="col-4">
							<button type="button" class="btn btn-block btn-info"
									onclick="location.href = base_url + 'Doctores/nuevo'">Registrarme como Doctor
							</button>
						</div>
					<?php } ?>
					<?php if (!$datosPaciente || count($datosPaciente) == 0) { ?>
						<div class="col-4">
							<button type="button" class="btn btn-block btn-info"
									onclick="location.href = base_url + 'Pacientes/nuevo'">Completar registro como
								Paciente
							</button>
						</div>
					<?php } else { ?>
						<div class="col-4">
							<button type="button" class="btn btn-block btn-warning"
									onclick="location.href = base_url + 'Atencion/nuevacita'">Reservar cita
							</button>
						</div>
						<div class="col-4" style="display: none;">
							<button type="button" class="btn btn-block btn-info"
									onclick="location.href = base_url + 'Pacientes/actualizar'">Actualizar datos
								Paciente
							</button>
						</div>
					<?php } ?>
										...
										if ($citasDoctor && count($citasDoctor) > 0) {
											foreach ($citasDoctor as $citasd) {
												...
												<tr>
													...
													<td><?= $citasd->dia ?></td>
													<td><?= $citasd->hora ?></td>
													<td><?= ($citasd->valido == 1) ? "<a target='_blank' href='" . base_url('Atencion/call') . "/$idPac/$idCita/doctor.html?room=" . md5($citasd->idPaciente) . "' class='btn btn-success'>Acceder</a>" : 'Enlace no activo'; ?></td>
												</tr>
												<?php
											}
										}
								...
								<?php
									foreach ($citasPaciente as $citasd) {
										...
										<tr>
											...
											<td><?= $citasd->dia ?></td>
											<td><?= $citasd->hora ?></td>
											<td><?= ($citasd->valido == 1) ? "<a target='_blank' href='https://call.doctoraunclick.com/comm.html?room=" . md5($citasd->idPaciente) . "' class='btn btn-success'>Acceder</a>" : 'Enlace no activo'; ?></td>
										</tr>
										<?php
									}
								?>
