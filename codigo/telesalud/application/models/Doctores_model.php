<?php
	...
	class Doctores_model extends CI_Model
	{
		...
		{
			$this->db->select("CIT.idCita, concat(PAC.nombres,' ',PAC.apellidos) as paciente, 
		CIT.fechaCita as dia, CIT.horaInicio as hora, CIT.enlaceAtencion as enlace ", false);
			$this->db->from("pac_citas CIT");
			$this->db->join('usr_paciente PAC', 'CIT.idPaciente = PAC.idPaciente', 'LEFT');
		...
		{
			$this->db->select("CIT.idCita, concat(PAC.nombres,' ',PAC.apellidos) as paciente, PAC.idPaciente,
		...
		{
			$this->db->select("CIT.idCita, concat(PAC.nombres,' ',PAC.apellidos) as paciente, PAC.idPaciente,
			concat(DOC.nombre_primer,' ',DOC.apellido_primer) as doctor,
			CIT.fechaCita as dia, CIT.horaInicio as hora, CIT.enlaceAtencion as enlace, if(CIT.atendido=0 AND CIT.horaInicio <= CURRENT_TIME(), 1, 0) as valido", false);
			$this->db->from("pac_citas CIT");
			$this->db->join('usr_paciente PAC', 'CIT.idPaciente = PAC.idPaciente', 'LEFT');
			$this->db->join('usr_usuarios DOC', 'CIT.idDoctor = DOC.idUser', 'LEFT');
			$this->db->where("CIT.idUser", $idPac);
			$this->db->where("CIT.atendido", 0);
			$this->db->where("CIT.fechaCita = CURDATE()");
